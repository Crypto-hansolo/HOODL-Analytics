...
type IndexedState = { holders: number | null; priceUsd: string | null; volume24h: string | null; transfers: Awaited<ReturnType<typeof getTokenTransfers>>; error: string | null }
...
function Chart({ title, subtitle }: { title: string; subtitle: string }) {
  return <section className="panel chart-panel"><div className="panel-head"><div><h2>{title}</h2><p>{subtitle}</p></div><div className="range"><button>1H</button><button className="selected">24H</button><button>7D</button><button>30D</button><button>90D</button><button>ALL</button></div></div><div className="chart-empty"><div><span>Historical series unavailable</span><small>No verified time-series snapshot is configured. Live contract reads are intentionally not presented as historical data.</small></div></div></section>
}
...
function Overview({ token, chain, indexed }: { token: TokenState; chain: ChainState; indexed: IndexedState }) {
  return <>
    ...
    <div className="two-col"><Chart title="Trading activity" subtitle="Historical ranges · indexed snapshots only" /><section className="panel status-panel"><div className="panel-head"><div><h2>Data integrity</h2><p>What is verified right now</p></div><Badge tone="live">TRANSPARENT</Badge></div><div className="status-row"><span><i className="dot green" />Token contract</span><b>{token.error ? 'Unavailable' : 'Configured'}</b></div><div className="status-row"><span><i className="dot green" />Network identity</span><b>{chain.chainId === CHAIN.id ? 'Confirmed' : 'Pending'}</b></div><div className="status-row"><span><i className="dot green" />Transfer activity</span><b>{indexed.transfers.length ? `${indexed.transfers.length} recent events` : 'Unavailable'}</b></div><div className="status-row"><span><i className="dot gray" />Fees / WETH rewards</span><b>Unavailable</b></div><div className="source-note">Sources: RPC for contract reads; Blockscout for indexed token metadata and transfers. Historical series, rewards, and pool-derived economics remain unavailable until verified.</div></section></div>
...
function App() {
  const [tab, setTab] = useState<Tab>('Overview'); const [token, setToken] = useState<TokenState>({ name: null, symbol: null, decimals: null, supply: null, error: null }); const [chain, setChain] = useState<ChainState>({ chainId: null, block: null, deployed: null, error: null }); const [poolV3, setPoolV3] = useState<PoolV3Data | null>(null); const [quote, setQuote] = useState<PoolSpotQuote | null>(null); const [indexed, setIndexed] = useState<IndexedState>({ holders: null, priceUsd: null, volume24h: null, transfers: [], error: null }); const [updated, setUpdated] = useState(Date.now())
  async function load() { try { const [name, symbol, decimals, supply] = await Promise.all([erc20Name(HOODL_TOKEN.address), erc20Symbol(HOODL_TOKEN.address), erc20Decimals(HOODL_TOKEN.address), erc20TotalSupply(HOODL_TOKEN.address)]); setToken({ name, symbol, decimals, supply, error: null }) } catch (err) { setToken(v => ({ ...v, error: err instanceof Error ? err.message : 'RPC unavailable' })) } try { const [chainId, block, code] = await Promise.all([ethChainId(), ethBlockNumber(), ethGetCode(HOODL_TOKEN.address)]); setChain({ chainId, block, deployed: code !== '0x', error: null }) } catch (err) { setChain(v => ({ ...v, error: err instanceof Error ? err.message : 'RPC unavailable' })) } try { const [info, transfers] = await Promise.all([getTokenInfo(HOODL_TOKEN.address), getTokenTransfers(HOODL_TOKEN.address, 50)]); setIndexed({ holders: info.holdersCount, priceUsd: info.exchangeRateUsd, volume24h: info.volume24h, transfers, error: null }) } catch (err) { setIndexed(v => ({ ...v, error: err instanceof Error ? err.message : 'Blockscout unavailable' })) } try { const livePool = await fetchPoolV3Data(CONFIGURED_POOL.address); setPoolV3(livePool); const token0 = livePool.token0.value; const token1 = livePool.token1.value; const slot0 = livePool.slot0.value; if (livePool.token0.status !== 'on-chain' || livePool.token1.status !== 'on-chain' || livePool.slot0.status !== 'on-chain' || !token0 || !token1 || !slot0) throw new Error('Pool identity or slot0 unavailable'); const [symbol0, symbol1, decimals0, decimals1] = await Promise.all([erc20Symbol(token0), erc20Symbol(token1), erc20Decimals(token0), erc20Decimals(token1)]); const isHoodl0 = token0.toLowerCase() === HOODL_TOKEN.address.toLowerCase() && symbol1?.toUpperCase() === 'WETH'; const isHoodl1 = token1.toLowerCase() === HOODL_TOKEN.address.toLowerCase() && symbol0?.toUpperCase() === 'WETH'; if ((!isHoodl0 && !isHoodl1) || !symbol0 || !symbol1 || slot0.sqrtPriceX96 === 0n) throw new Error('Pool is not verified as HOODL/WETH'); const rawToken1PerToken0 = Number(slot0.sqrtPriceX96) ** 2 / 2 ** 192 * 10 ** (decimals0 - decimals1); const wethPerHoodl = isHoodl0 ? rawToken1PerToken0 : 1 / rawToken1PerToken0; if (!Number.isFinite(wethPerHoodl)) throw new Error('Derived spot price is not finite'); setQuote({ status: 'on-chain', wethPerHoodl, token0Symbol: symbol0, token1Symbol: symbol1, error: null }) } catch (err) { setQuote({ status: 'unavailable', wethPerHoodl: null, token0Symbol: null, token1Symbol: null, error: err instanceof Error ? err.message : 'Pool quote unavailable' }) } setUpdated(Date.now()) }
