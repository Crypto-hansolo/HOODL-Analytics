...
type IndexedState = { holders: number | null; priceUsd: string | null; volume24h: string | null; transfers: Awaited<ReturnType<typeof getTokenTransfers>>; activity: ActivityCounts | null; holderRows: BlockscoutHolder[]; snapshotAt: string | null; snapshotStale: boolean; error: string | null }

type Snapshot = { generatedAt: string; transfers: Awaited<ReturnType<typeof getTokenTransfers>>; activity?: ActivityCounts }
...
function Chart({ title, subtitle, activity }: { title: string; subtitle: string; activity?: ActivityCounts | null }) {
  const ranges = ['1H', '24H', '7D', '30D', '90D', 'ALL'] as const
  const [range, setRange] = useState<(typeof ranges)[number]>('24H')
  const count = activity && range === '1H' ? activity.transfers1h : activity && range === '24H' ? activity.transfers24h : activity && range === '7D' ? activity.transfers7d : null
  return <section className="panel chart-panel"><div className="panel-head"><div><h2>{title}</h2><p>{subtitle}</p></div><div className="range" aria-label="Historical time range">{ranges.map((item) => <button key={item} className={range === item ? 'selected' : ''} aria-pressed={range === item} onClick={() => setRange(item)}>{item}</button>)}</div></div><div className={count !== null ? 'chart-value' : 'chart-empty'}><div>{count !== null ? <><span>{count} verified token transfers · {range}</span><small>Bounded Blockscout snapshot count, not a pool swap or volume series.</small></> : <><span>Historical series unavailable · {range}</span><small>No verified snapshot covers this range. Live contract reads are intentionally not presented as historical data.</small></>}</div></div></section>
}
...
function Overview({ token, chain, indexed }: { token: TokenState; chain: ChainState; indexed: IndexedState }) {
  return <>
    ...
    <div className="two-col"><Chart title="Transfer activity" subtitle={`Verified token transfers · bounded snapshot${indexed.snapshotStale ? ' · stale' : ''}`} activity={indexed.activity} /><section className="panel status-panel"><div className="panel-head"><div><h2>Data integrity</h2><p>What is verified right now</p></div><Badge tone={indexed.snapshotStale ? 'warn' : 'live'}>{indexed.snapshotStale ? 'STALE SNAPSHOT' : 'TRANSPARENT'}</Badge></div><div className="status-row"><span><i className="dot green" />Token contract</span><b>{token.error ? 'Unavailable' : 'Configured'}</b></div><div className="status-row"><span><i className="dot green" />Network identity</span><b>{chain.chainId === CHAIN.id ? 'Confirmed' : 'Pending'}</b></div><div className="status-row"><span><i className="dot green" />Transfer activity</span><b>{indexed.transfers.length ? `${indexed.transfers.length} recent events` : 'Unavailable'}</b></div><div className="status-row"><span><i className="dot gray" />Fees / WETH rewards</span><b>Unavailable</b></div><div className="source-note">Sources: RPC for contract reads; Blockscout for indexed token metadata and transfers. Pool swaps, historical volume, rewards, and pool-derived economics remain unavailable until verified.{indexed.snapshotAt ? ` Snapshot generated ${new Date(indexed.snapshotAt).toLocaleString()}${indexed.snapshotStale ? ' (stale; rerun the snapshot workflow to regenerate it).' : '.'}` : ' Snapshot unavailable.'}</div></section></div>
...
function TabContent({ tab, token, chain, poolV3, quote, indexed }: { tab: Tab; token: TokenState; chain: ChainState; poolV3: PoolV3Data | null; quote: PoolSpotQuote | null; indexed: IndexedState }) {
  ...
  if (tab === 'Trading') return <><div className="metrics"><Metric label="Verified transfers" value={indexed.transfers.length ? formatInteger(indexed.transfers.length) : '—'} source={indexed.transfers.length ? 'Blockscout' : 'Unavailable'} /><Metric label="24h activity" value={indexed.activity ? formatInteger(indexed.activity.transfers24h) : '—'} source={indexed.activity ? 'Calculated' : 'Unavailable'} /><Metric label="Total volume" value="—" /><Metric label="Unique traders" value="—" /></div><Chart title="Transfer activity" subtitle="Verified token transfers · bounded source coverage" activity={indexed.activity} /><TransferActivity transfers={indexed.transfers} decimals={token.decimals} stale={indexed.snapshotStale} error={indexed.error} /></>
...
function App() {
  ...
  async function load() {
    ...
    try {
      ...
      setIndexed({ holders: info?.holdersCount ?? null, priceUsd: info?.exchangeRateUsd ?? null, volume24h: info?.volume24h ?? null, transfers: liveTransfers.length ? liveTransfers : snapshot?.transfers ?? [], activity: snapshot?.activity ?? null, holderRows, snapshotAt, snapshotStale, error: failures.length ? `${failures.length} indexed source${failures.length === 1 ? '' : 's'} unavailable` : null })
