...
function EmptyState({ title, body, todo, onRetry, retrying }: { title: string; body: string; todo: string; onRetry?: () => void; retrying?: boolean }) {
  return <div className="empty"><div className="empty-icon">◌</div><h3>{title}</h3><p>{body}</p><small className="empty-next"><span>Next verified step</span> {todo}</small>{onRetry && <button type="button" className="empty-retry" onClick={onRetry} disabled={retrying}>{retrying ? 'Retrying…' : 'Retry now ↻'}</button>}</div>
}
...
function PoolsTab({ poolV3, quote, swaps, decimals, onRetry, retrying }: { poolV3: PoolV3Data | null; quote: PoolSpotQuote | null; swaps: PoolSwap[]; decimals: number | null; onRetry: () => void; retrying: boolean }) {
...
function HoldersTab({ indexed, token, onRetry, retrying }: { indexed: IndexedState; token: TokenState; onRetry: () => void; retrying: boolean }) {
