...
function Chart({ title, subtitle, activity, coverageComplete7d = false }: { title: string; subtitle: string; activity?: ActivityCounts | null; coverageComplete7d?: boolean }) {
  ...
  const maxCount = Math.max(...verifiedCounts, 1)
  const barWidth = count === null ? 0 : Math.max(6, Math.round((count / maxCount) * 100))
  return <section className="panel chart-panel"><div className="panel-head"><div><h2>{title}</h2><p>{subtitle}</p></div><div className="range" aria-label="Historical time range">{ranges.map((item) => <button key={item} className={range === item ? 'selected' : ''} aria-pressed={range === item} onClick={() => setRange(item)}>{item}</button>)}</div></div><div className={count !== null ? 'chart-value' : 'chart-empty'}>{count !== null ? <div className="chart-reading"><div className="chart-reading-head"><span>{count} verified token transfers · {range}</span><strong>{barWidth}%</strong></div><div className="activity-track" role="img" aria-label={`${count} verified token transfers in the ${range} range, relative to the largest verified range`}><span style={{ width: `${barWidth}%` }} /></div><small>Relative to the largest verified range in this snapshot ({maxCount} transfers). This is not a pool swap or volume series.</small></div> : <div><span>Historical series unavailable · {range}</span><small>{coverageComplete7d ? 'No verified snapshot covers this range. Live contract reads are intentionally not presented as historical data.' : 'Snapshot coverage did not reach the 7-day boundary; counts are withheld rather than shown as incomplete.'}</small></div>}</div></section>
}
