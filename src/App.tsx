...
type IndexedState = { holders: number | null; priceUsd: string | null; volume24h: string | null; transfers: Awaited<ReturnType<typeof getTokenTransfers>>; activity: ActivityCounts | null; holderRows: BlockscoutHolder[]; snapshotAt: string | null; error: string | null }
...
function Overview({ token, chain, indexed }: { token: TokenState; chain: ChainState; indexed: IndexedState }) {
  return <>
    ...
    <div className="two-col"><Chart title="Transfer activity" subtitle="Verified token transfers · bounded snapshot" activity={indexed.activity} /><section className="panel status-panel"><div className="panel-head"><div><h2>Data integrity</h2><p>What is verified right now</p></div><Badge tone="live">TRANSPARENT</Badge></div><div className="status-row"><span><i className="dot green" />Token contract</span><b>{token.error ? 'Unavailable' : 'Configured'}</b></div><div className="status-row"><span><i className="dot green" />Network identity</span><b>{chain.chainId === CHAIN.id ? 'Confirmed' : 'Pending'}</b></div><div className="status-row"><span><i className="dot green" />Transfer activity</span><b>{indexed.transfers.length ? `${indexed.transfers.length} recent events` : 'Unavailable'}</b></div><div className="status-row"><span><i className="dot gray" />Fees / WETH rewards</span><b>Unavailable</b></div><div className="source-note">Sources: RPC for contract reads; Blockscout for indexed token metadata and transfers. Pool swaps, historical volume, rewards, and pool-derived economics remain unavailable until verified.{indexed.snapshotAt ? ` Snapshot generated ${new Date(indexed.snapshotAt).toLocaleString()}.` : ' Snapshot unavailable.'}</div></section></div>
...
function App() {
  const [tab, setTab] = useState<Tab>('Overview'); const [token, setToken] = useState<TokenState>({ name: null, symbol: null, decimals: null, supply: null, error: null }); const [chain, setChain] = useState<ChainState>({ chainId: null, block: null, deployed: null, error: null }); const [poolV3, setPoolV3] = useState<PoolV3Data | null>(null); const [quote, setQuote] = useState<PoolSpotQuote | null>(null); const [indexed, setIndexed] = useState<IndexedState>({ holders: null, priceUsd: null, volume24h: null, transfers: [], activity: null, holderRows: [], snapshotAt: null, error: null }); const [updated, setUpdated] = useState(Date.now()); const [loading, setLoading] = useState(false)
  async function load() {
    setLoading(true)
    try {
    ...
    try {
      ...
      setIndexed({ holders: info.holdersCount, priceUsd: info.exchangeRateUsd, volume24h: info.volume24h, transfers: snapshot.transfers.length ? snapshot.transfers : transfers, activity: snapshot.activity ?? null, holderRows, snapshotAt: snapshot.generatedAt, error: null })
